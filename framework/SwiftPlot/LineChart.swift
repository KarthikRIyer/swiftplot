...
extension LineGraph{
    ...
    func calcMarkerLocAndScalePts(renderer: Renderer){
        ...
        while xM<=plotDimensions.graphWidth {
            ...
            let text_p: Point = Point(xM - (renderer.getTextWidth(text: "\(round(primaryAxis.scaleX*(xM-originPrimary.x)))", textSize: primaryAxis.plotMarkers.markerTextSize)/2.0) + 8, -15)
        ...
        while yM<=plotDimensions.graphHeight {
            ...
            let text_p: Point = Point(-(renderer.getTextWidth(text: "\(round(primaryAxis.scaleY*(yM-originPrimary.y)))", textSize: primaryAxis.plotMarkers.markerTextSize)+5), yM - 4)
        ...
        if (secondaryAxis != nil) {
            ...
            while yM<=plotDimensions.graphHeight {
                ...
                let text_p: Point = Point(plotDimensions.graphWidth + (renderer.getTextWidth(text: "\(round(secondaryAxis!.scaleY*(yM-originSecondary!.y)))", textSize: secondaryAxis!.plotMarkers.markerTextSize)*Float(0.5) - 5), yM - 4)
    ...
    func drawMarkers(renderer: Renderer) {
        for index in 0..<primaryAxis.plotMarkers.xMarkers.count {
            ...
            renderer.drawText(text: primaryAxis.plotMarkers.xMarkersText[index], location: primaryAxis.plotMarkers.xMarkersTextLocation[index], textSize: primaryAxis.plotMarkers.markerTextSize, strokeWidth: 0.7, angle: 0, isOriginShifted: true)
        }
        ...
        for index in 0..<primaryAxis.plotMarkers.yMarkers.count {
            ...
            renderer.drawLine(startPoint: p1, endPoint: p2, strokeWidth: plotBorder.borderThickness, strokeColor: Color.black, isDashed: false, isOriginShifted: true)
            renderer.drawText(text: primaryAxis.plotMarkers.yMarkersText[index], location: primaryAxis.plotMarkers.yMarkersTextLocation[index], textSize: primaryAxis.plotMarkers.markerTextSize, strokeWidth: 0.7, angle: 0, isOriginShifted: true)
        }
        ...
        if (secondaryAxis != nil) {
            for index in 0..<secondaryAxis!.plotMarkers.yMarkers.count {
                ...
                renderer.drawLine(startPoint: p1, endPoint: p2, strokeWidth: plotBorder.borderThickness, strokeColor: Color.black, isDashed: false, isOriginShifted: true)
                renderer.drawText(text: secondaryAxis!.plotMarkers.yMarkersText[index], location: secondaryAxis!.plotMarkers.yMarkersTextLocation[index], textSize: secondaryAxis!.plotMarkers.markerTextSize, strokeWidth: 0.7, angle: 0, isOriginShifted: true)
            }
        }
    ...
    func drawLegends(renderer: Renderer) {
        ...
        for i in 0..<allSeries.count {
            ...
            renderer.drawSolidRect(topLeftPoint: tL, topRightPoint: tR, bottomRightPoint: bR, bottomLeftPoint: bL, fillColor: allSeries[i].color, hatchPattern: .none, isOriginShifted: false)
            let p: Point = Point(bR.x + plotLegend.legendTextSize, bR.y)
            renderer.drawText(text: allSeries[i].label, location: p, textSize: plotLegend.legendTextSize, strokeWidth: 1.2, angle: 0, isOriginShifted: false)
        }
