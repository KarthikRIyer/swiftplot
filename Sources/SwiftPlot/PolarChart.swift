...
public struct PolarGraph<T:FloatConvertible,U:FloatConvertible>: Plot {
    ...
    public var reference = Pair<T, U>(T(Float(0.0)), U(Float(1.0)))
    // Data.
...
// Setting data.
extension PolarGraph {

    public mutating func linspace(start: Float, end: Float, num: Int) -> Array<Float>{
      var increment = (end - start) / Float((num - 1))
      var current = start
      var outputs = [Float]()
      for i in 1...num {
    ...
    public mutating func addSeries(_ r: [T],
                          _ theta: [U],
                          label: String,
...
extension PolarGraph: HasGraphLayout {
    ...
    if let axisInfo = data.primaryAxisInfo {
      for dataset in primaryAxis.series {
        ...
        renderer.drawPlotLines(points: points,
                               ...
                               isDashed: false)
        renderer.drawEmptyCircle(center: pointOrigin, radius: Float(2.0)*Float(referenceRadius.y-pointOrigin.y))
      }
    }
